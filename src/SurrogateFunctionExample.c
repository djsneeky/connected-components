...
/****************************************************************************************************/
/* This program generates arrays of data for plots of the QGGMRF potential and
 * surrogate functions. */
/****************************************************************************************************/
int main(int argc, char **argv) {
  double delta_prime; /* initial difference between pixel x_s^\prime and its
                         neighbor x_r^\prime */
  double b;           /* neighborhood relationship coefficient. */
  double
      sigma_x; /* scale parameter. Increasing sigma_x reduces regularization. */
  double p; /* 1st shape parameter of qggmrf. Typically, p=1.2, and it is best
               to choose 1.0<p<=2.0 */
  ...
  /* This is a constant that is added to the surrogate so it will be tagent to
   * the potential function */
  offset = get_rho(delta_prime, b, sigma_x, p, q, T) -
           btilde * pow(delta_prime, 2.0);

  /* Generate array of rho and Q values used in plot */
  for (i = 0; i < NN; ++i) {
    rho[i] = get_rho(delta[i], b, sigma_x, p, q,
                     T); /* Compute value of potential function */
    Q[i] = btilde * pow(delta[i], 2.0) +
           offset; /* Compute value of surrogate function */
  }
