...
double get_btilde(
    ...
    double sigma_x,     /* scale parameter. Increasing sigma_x reduces
                           regularization. */
...
/* Computes the b_tilde parameter used for surrogate function minimization for
   the qGGMRF prior. */
{
  double b_tilde;
  double tmpa, tmpb, tmpc; /* temporary constants used in calculation */

  delta_prime = fabs(delta_prime) +
                sigma_x * DBL_EPSILON; /* this is for numberical stability */

  /* tmpa is the product of the first three terms reorganized for numberical
   * stability when q=0. */
  tmpa = b * pow(delta_prime, q - 2.0) /
         (2.0 * pow(sigma_x, p) * pow(T * sigma_x, q - p));
