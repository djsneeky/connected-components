...
static int32_t WriteIFD(FILE *fp, struct IFD *ifd, struct TIFF_header *header);
...
static void AllocateStripBuffer(uint8_t **buffer, struct DataLocation *DataLoc,
                                int32_t width);
static int32_t PutStrip(FILE *fp, struct TIFF_img *img,
                        struct DataLocation *DataLoc, uint8_t *strip_buf,
                        uint32_t strip_index);
static int32_t PackStrip(struct TIFF_img *img, struct DataLocation *DataLoc,
                         uint8_t *buffer, uint32_t strip_index);
static int32_t WriteStrip(FILE *fp, struct DataLocation *DataLoc,
                          uint8_t *strip_buf, uint32_t strip_index);
static int32_t MakeImageDataLocInfo(struct TIFF_img *img,
                                    struct DataLocation *DataLoc);
static int32_t ComputeStripsPerImage(int32_t height,
                                     struct DataLocation *DataLoc);
...
static int32_t PrepareHeader(struct TIFF_header *header,
                             struct DataLocation *DataLoc);
static int32_t PrepareIFD(struct TIFF_img *img, struct IFD *ifd,
                          struct DataLocation *DataLoc);
static int32_t AddSpecialFieldEntries(struct TIFF_img *img, struct IFD *ifd);
static int32_t AddGrayscaleFields(struct IFD *ifd, char TIFF_type);
static int32_t AddPaletteColorFields(struct TIFF_img *img, struct IFD *ifd);
...
static int32_t AddCoreFieldEntries(struct TIFF_img *img, struct IFD *ifd,
                                   struct DataLocation *DataLoc);
...
static int32_t MakeStripOffsetsField(struct IFD *ifd,
                                     struct DataLocation *DataLoc);
static int32_t MakePhotometricInterpretationField(struct IFD *ifd,
                                                  char TIFF_type);
static int32_t MakeCompressionField(struct IFD *ifd, char compress_type);
static int32_t MakeImageWidthOrLengthField(struct IFD *ifd, uint16_t Tag,
                                           int32_t dimension);
static int32_t MakeDefaultRowsPerStripField(struct IFD *ifd);
static int32_t MakeColorMapField(struct TIFF_img *img, struct IFD *ifd);
static uint32_t GetMaxValUL(uint32_t *array, uint32_t n_elements);
...
static int32_t GetStrip(FILE *fp, struct TIFF_img *img,
                        struct DataLocation *DataLoc, uint8_t *strip_buf,
                        uint32_t strip_index);
static int32_t UnpackStrip(struct TIFF_img *img, struct DataLocation *DataLoc,
                           uint32_t strip_index, uint8_t *buffer);
static int32_t ReadStrip(FILE *fp, struct DataLocation *DataLoc,
                         uint32_t strip_index, uint8_t *strip_buf);
static int32_t GetUShortValueFromField(struct IFD *ifd, uint16_t Tag,
                                       uint16_t *Value);
static int32_t GetImageDataLocInfo(struct IFD *ifd,
                                   struct DataLocation *DataLoc);
static int32_t GetStripOffsets(struct IFD *ifd, struct DataLocation *DataLoc);
static int32_t GetStripByteCounts(struct IFD *ifd,
                                  struct DataLocation *DataLoc);
static int32_t PutValuesIntoULongArray(struct TIFF_field *field,
                                       uint32_t **array, uint32_t num_elements);
static int32_t GetNumberOfStrips(struct IFD *ifd, uint32_t *StripsPerImage);
static int32_t GetRowsPerStrip(struct IFD *ifd, uint32_t *rows_per_strip);
static void WrongValueType(char *FunctionName, uint16_t Tag, uint16_t Type);
...
static int32_t GetHeightAndWidth(struct IFD *ifd, int32_t *height,
                                 int32_t *width);
...
static struct TIFF_field *GetFieldStructure(struct IFD *ifd, uint16_t Tag);
...
static int32_t ReadIFD(FILE *fp, struct IFD *ifd, struct TIFF_header *header,
                       char *TIFF_type);
...
static int32_t AllocateAndCopyField(FILE *fp, struct IFD *ifd, uint16_t Tag,
                                    uint16_t Type);
static int32_t IsTypeExpectedWithTag(uint16_t Tag, uint16_t Type);
static int32_t IsTypeRecognized(uint16_t Type);
static int32_t CopyField(FILE *fp, struct TIFF_field *field, uint16_t Tag,
                         uint16_t Type);
...
int32_t write_TIFF(FILE *fp, struct TIFF_img *img) {
  ...
  if (WriteImageData(fp, img, &(DataLoc)) == ERROR) return (ERROR);

  /* prepare header and image file directory */
  if (PrepareHeaderAndIFD(img, &(ifd), &(header), &(DataLoc)) == ERROR)
    return (ERROR);

  /* write header and image file directory */
  if (WriteHeaderAndIFD(fp, &(ifd), &(header)) == ERROR) return (ERROR);
...
int32_t get_TIFF(struct TIFF_img *img, int32_t height, int32_t width,
                 char TIFF_type) {
  int32_t i;
  ...
  if (img->TIFF_type == 'c') {
    img->color = (uint8_t ***)mget_spc(3, sizeof(uint8_t **));
    for (i = 0; i < 3; i++)
      img->color[i] =
          (uint8_t **)get_img(img->width, img->height, sizeof(uint8_t));
    return (NO_ERROR);
  }
...
static int32_t FreeIFD(struct IFD *ifd) {
  ...
  for (i = 0; i < ifd->NumberOfFields; i++)
    if (ifd->Fields[i].Count > 1)
      if (FreeFieldValues(&(ifd->Fields[i])) == ERROR) return (ERROR);
...
static int32_t FreeFieldValues(struct TIFF_field *field) {
  ...
  fprintf(stderr, "not prepared to free value array of type %d", field->Type);
  return (ERROR);
}
...
static int32_t WriteIFD(FILE *fp, struct IFD *ifd, struct TIFF_header *header) {
  ...
  outside_IFD_offset =
      header->OffsetOfFirstIFD + 2 + (uint32_t)(ifd->NumberOfFields * 12) + 4;

  /* set file position */
  if (SetFilePosition(fp, header->OffsetOfFirstIFD) == ERROR) {
  ...
  if (WriteUnsignedShort(fp, &(ifd->NumberOfFields)) == ERROR) return (ERROR);
  ...
  zero = 0;
  if (WriteUnsignedLong(fp, &(zero)) == ERROR) return (ERROR);
...
static int32_t WriteField(FILE *fp, struct TIFF_field *field,
  ...
  if (SetFilePosition(fp, starting_offset) == ERROR) return (ERROR);

  /* write tag */
  if (WriteUnsignedShort(fp, &(field->Tag)) == ERROR) return (ERROR);

  /* write type */
  if (WriteUnsignedShort(fp, &(field->Type)) == ERROR) return (ERROR);

  /* write Count */
  if (WriteUnsignedLong(fp, &(field->Count)) == ERROR) return (ERROR);
  ...
  if (WriteValue(fp, field, outside_IFD_offset) == ERROR) return (ERROR);
...
static int32_t WriteValue(FILE *fp, struct TIFF_field *field,
  ...
  if (4 < field->Count * field->SizeOfType) {
    ...
    if (WriteUnsignedLong(fp, outside_IFD_offset) == ERROR) return (ERROR);
...
static int32_t WriteArrayOfValues(FILE *fp, struct TIFF_field *field) {
  ...
  if (field->Type == BYTE) {
    for (i = 0; i < field->Count; i++)
      if (WriteUnsignedChar(fp, &(field->Value.UCharArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == SHORT) {
    for (i = 0; i < field->Count; i++)
      if (WriteUnsignedShort(fp, &(field->Value.UShortArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == LONG) {
    for (i = 0; i < field->Count; i++)
      if (WriteUnsignedLong(fp, &(field->Value.ULongArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == RATIONAL) {
    for (i = 0; i < field->Count; i++)
      if (WriteRational(fp, &(field->Value.RatArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  }
...
static int32_t WriteSingleValue(FILE *fp, struct TIFF_field *field) {
  if (field->Type == BYTE) {
    if (WriteUnsignedChar(fp, &(field->Value.UChar)) == ERROR) return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == SHORT) {
    if (WriteUnsignedShort(fp, &(field->Value.UShort)) == ERROR) return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == LONG) {
    if (WriteUnsignedLong(fp, &(field->Value.ULong)) == ERROR) return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == RATIONAL) {
    if (WriteRational(fp, &(field->Value.Rat)) == ERROR) return (ERROR);
    return (NO_ERROR);
  }
...
static int32_t WriteHeader(FILE *fp, struct TIFF_header *header) {
  ...
  if (WriteUnsignedShort(fp, &(header->ByteOrder)) == ERROR) return (ERROR);

  /* write forty-two */
  if (WriteUnsignedShort(fp, &(header->FortyTwo)) == ERROR) return (ERROR);

  /* write offset of first IFD */
  if (WriteUnsignedLong(fp, &(header->OffsetOfFirstIFD)) == ERROR)
    return (ERROR);
...
static int32_t WriteRational(FILE *fp, struct Rational *Rat) {
  ...
  if ((1 != fwrite(&numerator, sizeof(uint32_t), 1, fp)) ||
      (1 != fwrite(&denumerator, sizeof(uint32_t), 1, fp))) {
...
static int32_t WriteUnsignedLong(FILE *fp, uint32_t *UnsignedLong) {
  ...
  if (1 != fwrite((uint32_t *)&unsignedlong, sizeof(uint32_t), 1, fp)) {
...
static int32_t WriteUnsignedShort(FILE *fp, uint16_t *UnsignedShort) {
  ...
  if (1 != fwrite((uint16_t *)&unsignedshort, sizeof(uint16_t), 1, fp)) {
...
static int32_t WriteUnsignedChar(FILE *fp, uint8_t *UnsignedChar) {
  if (1 != fwrite((uint8_t *)UnsignedChar, sizeof(uint8_t), 1, fp)) {
...
static int32_t WriteImageData(FILE *fp, struct TIFF_img *img,
  ...
  if (MakeImageDataLocInfo(img, DataLoc) == ERROR) return (ERROR);
...
static void FreeStripBuffer(uint8_t *buffer) { free((void *)buffer); }
...
static void AllocateStripBuffer(uint8_t **buffer, struct DataLocation *DataLoc,
  ...
  *buffer = (uint8_t *)mget_spc((int32_t)upper_bd_on_bytes_in_one_strip,
                                sizeof(uint8_t));
}
...
static int32_t PutStrip(FILE *fp, struct TIFF_img *img,
  ...
  if (WriteStrip(fp, DataLoc, strip_buf, strip_index) == ERROR) return (ERROR);
...
static int32_t WriteStrip(FILE *fp, struct DataLocation *DataLoc,
  ...
  if (SetFilePosition(fp, DataLoc->strip_offsets[strip_index]) == ERROR)
    return (ERROR);
...
static int32_t MakeImageDataLocInfo(struct TIFF_img *img,
  ...
  if (ComputeStripsPerImage(img->height, DataLoc) == ERROR) return (ERROR);

  /* allocate arrays for DataLoc structure */
  DataLoc->strip_byte_counts =
      (uint32_t *)mget_spc((int32_t)DataLoc->StripsPerImage, sizeof(uint32_t));
  DataLoc->strip_offsets =
      (uint32_t *)mget_spc((int32_t)DataLoc->StripsPerImage, sizeof(uint32_t));
...
static int32_t ComputeStripsPerImage(int32_t height,
  ...
  if ((ULheight % DataLoc->rows_per_strip) == 0)
    ...
    DataLoc->StripsPerImage = 1 + (uint32_t)(height / DataLoc->rows_per_strip);
...
static int32_t PrepareHeader(struct TIFF_header *header,
  ...
  if ((header->OffsetOfFirstIFD % 2) != 0) header->OffsetOfFirstIFD++;
...
static int32_t PrepareIFD(struct TIFF_img *img, struct IFD *ifd,
  ...
  if (AddCoreFieldEntries(img, ifd, DataLoc) == ERROR) return (ERROR);

  /* add fields which are required */
  /* for this specific image type  */
  if (AddSpecialFieldEntries(img, ifd) == ERROR) return (ERROR);
...
static int32_t AddSpecialFieldEntries(struct TIFF_img *img, struct IFD *ifd) {
  /* for grayscale image */
  if (img->TIFF_type == 'g') return (AddGrayscaleFields(ifd, img->TIFF_type));

  /* for palette-color image */
  if (img->TIFF_type == 'p') return (AddPaletteColorFields(img, ifd));

  /* for color image */
  if (img->TIFF_type == 'c') return (AddColorFields(ifd, img->TIFF_type));
...
static int32_t AddGrayscaleFields(struct IFD *ifd, char TIFF_type) {
  ...
  if (MakeBitsPerSampleField(ifd, TIFF_type) == ERROR) return (ERROR);
...
static int32_t AddPaletteColorFields(struct TIFF_img *img, struct IFD *ifd) {
  ...
  if (MakeBitsPerSampleField(ifd, img->TIFF_type) == ERROR) return (ERROR);
...
static int32_t AddColorFields(struct IFD *ifd, char TIFF_type) {
  ...
  if (MakeSamplesPerPixelField(ifd) == ERROR) return (ERROR);
...
static int32_t SortFields(struct IFD *ifd) {
  ...
  for (i = 0; i < ifd->NumberOfFields - 1; i++) {
    ...
    if (min_tag_index != i)
      SwitchFields(&(ifd->Fields[i]), &(ifd->Fields[min_tag_index]));
  }
...
static int32_t AddCoreFieldEntries(struct TIFF_img *img, struct IFD *ifd,
  ...
  if (MakeImageWidthOrLengthField(ifd, ImageWidth, img->width) == ERROR)
    return (ERROR);

  /* add ImageLength field */
  if (MakeImageWidthOrLengthField(ifd, ImageLength, img->height) == ERROR)
    return (ERROR);

  /* add Compression field */
  if (MakeCompressionField(ifd, img->compress_type) == ERROR) return (ERROR);

  /* add PhotometricInterpretation field */
  if (MakePhotometricInterpretationField(ifd, img->TIFF_type) == ERROR)
    return (ERROR);

  /* add StripOffsets field */
  if (MakeStripOffsetsField(ifd, DataLoc) == ERROR) return (ERROR);

  /* add RowsPerStrip field */
  if (MakeRowsPerStripField(ifd, DataLoc) == ERROR) return (ERROR);

  /* add StripByteCounts field */
  if (MakeStripByteCountsField(ifd, DataLoc) == ERROR) return (ERROR);

  /* add XResolution field */
  if (MakeResolutionField(ifd, XResolution) == ERROR) return (ERROR);

  /* add YResolution field */
  if (MakeResolutionField(ifd, YResolution) == ERROR) return (ERROR);
...
static int32_t MakeStripByteCountsField(struct IFD *ifd,
  ...
  if (max_bytes_in_one_strip < 65536) {
    ...
    if (field->Count == 1)
      field->Value.UShort = (uint16_t)DataLoc->strip_byte_counts[0];
    else {
      if (AllocateArrayOfValues(field) == ERROR) return (ERROR);
      for (i = 0; i < field->Count; i++)
        field->Value.UShortArray[i] = (uint16_t)DataLoc->strip_byte_counts[i];
    }
  } else {
    field->Type = LONG;
...
static int32_t MakePhotometricInterpretationField(struct IFD *ifd,
  ...
  if (TIFF_type == 'g')
    field->Value.UShort = BlackIsZero;
  else if (TIFF_type == 'p')
    field->Value.UShort = PaletteColor;
  else if (TIFF_type == 'c')
    field->Value.UShort = RGB;
  else {
...
static int32_t MakeCompressionField(struct IFD *ifd, char compress_type) {
  ...
  if (compress_type == 'u')
    field->Value.UShort = NoCompression;
  else {
...
static int32_t MakeDefaultRowsPerStripField(struct IFD *ifd) {
  ...
  if (GetHeightAndWidth(ifd, &(height), &(width)) == ERROR) return (ERROR);
...
static int32_t GetImageData(FILE *fp, struct TIFF_img *img, struct IFD *ifd) {
  ...
  if (GetCompression(ifd, &(img->compress_type)) == ERROR) return (ERROR);

  /* record image height and width */
  if (GetHeightAndWidth(ifd, &(img->height), &(img->width)) == ERROR)
    return (ERROR);
...
static int32_t ReadImageData(FILE *fp, struct TIFF_img *img, struct IFD *ifd) {
  ...
  if (img->TIFF_type == 'p')
    if (PutColorMapValuesIntoTable(img, ifd) == ERROR) return (ERROR);
  ...
  for (i = 0; i < DataLoc.StripsPerImage; i++)
    if (GetStrip(fp, img, &(DataLoc), strip_buf, i) == ERROR) return (ERROR);
...
static int32_t PutColorMapValuesIntoTable(struct TIFF_img *img,
  ...
  if ((field = GetFieldStructure(ifd, ColorMap)) == NULL) return (ERROR);
  ...
  for (i = 0; i < cmap_length; i++) {
    /* red entry */
    cp = (uint8_t *)(&(field->Value.UShortArray[i]));
    img->cmap[i][0] = *cp;

    /* green entry */
    cp = (uint8_t *)(&(field->Value.UShortArray[i + cmap_length]));
    img->cmap[i][1] = *cp;

    /* blue entry */
    cp =
        (uint8_t *)(&(field->Value.UShortArray[i + cmap_length + cmap_length]));
    img->cmap[i][2] = *cp;
  }
...
static int32_t GetStrip(FILE *fp, struct TIFF_img *img,
                        struct DataLocation *DataLoc, uint8_t *strip_buf,
                        uint32_t strip_index) {
  if (ReadStrip(fp, DataLoc, strip_index, strip_buf) == ERROR) return (ERROR);
...
static int32_t ReadStrip(FILE *fp, struct DataLocation *DataLoc,
  ...
  if (DataLoc->strip_byte_counts[strip_index] !=
      fread(strip_buf, sizeof(uint8_t), DataLoc->strip_byte_counts[strip_index],
            fp)) {
...
static int32_t GetUShortValueFromField(struct IFD *ifd, uint16_t Tag,
  ...
  if ((field = GetFieldStructure(ifd, Tag)) == NULL) return (ERROR);

  if (field->Type == SHORT)
    *Value = field->Value.UShort;
  else {
...
static int32_t GetImageDataLocInfo(struct IFD *ifd,
  ...
  if (GetRowsPerStrip(ifd, &(DataLoc->rows_per_strip)) == ERROR) return (ERROR);
  ...
  if (GetStripOffsets(ifd, DataLoc) == ERROR) return (ERROR);

  /* get array of strip byte counts */
  if (GetStripByteCounts(ifd, DataLoc) == ERROR) return (ERROR);
...
static int32_t GetStripOffsets(struct IFD *ifd, struct DataLocation *DataLoc) {
  ...
  if ((field = GetFieldStructure(ifd, StripOffsets)) == NULL) return (ERROR);
...
static int32_t GetStripByteCounts(struct IFD *ifd,
  ...
  if ((field = GetFieldStructure(ifd, StripByteCounts)) == NULL) return (ERROR);
...
static int32_t PutValuesIntoULongArray(struct TIFF_field *field,
  ...
  /* allocate array */
  if ((*array = (uint32_t *)calloc((size_t)num_elements, sizeof(uint32_t))) ==
      NULL) {
  ...
  if (field->Count != num_elements) {
    ...
    fprintf(stderr, "field count (%ld) for tag %d ", (long int)field->Count,
            field->Tag);
  ...
  if (field->Type == SHORT) {
    ...
    if (field->Count == 1)
      (*array)[0] = field->Value.ULong;
    else {
      for (i = 0; i < num_elements; i++)
        (*array)[i] = field->Value.ULongArray[i];
    }
  } else {
    WrongValueType("PutValuesIntoULongArray", field->Tag, field->Type);
    return (ERROR);
  }
...
static int32_t GetRowsPerStrip(struct IFD *ifd, uint32_t *rows_per_strip) {
  ...
  if ((field = GetFieldStructure(ifd, RowsPerStrip)) == NULL) return (ERROR);

  /* get number of rows per strip */
  if (field->Type == SHORT)
    *rows_per_strip = (uint32_t)field->Value.UShort;
  else if (field->Type == LONG)
    *rows_per_strip = field->Value.ULong;
  else {
...
static int32_t AllocateImageDataArray(struct TIFF_img *img, struct IFD *ifd) {
  ...
  fprintf(stderr, "space allocation for image of type %c ", img->TIFF_type);
...
static int32_t AllocateColorMap(uint8_t ***cmap, struct IFD *ifd) {
  ...
  if (GetUShortValueFromField(ifd, BitsPerSample, &bits_per_sample) == ERROR)
    return (ERROR);
  ...
  *cmap = (uint8_t **)get_img(3, cmap_length, sizeof(uint8_t));
...
static int32_t GetHeightAndWidth(struct IFD *ifd, int32_t *height,
  ...
  if ((field = GetFieldStructure(ifd, ImageLength)) == NULL) return (ERROR);

  /* get image height */
  if (field->Type == SHORT)
    *height = (int32_t)field->Value.UShort;
  else if (field->Type == LONG)
    *height = (int32_t)field->Value.ULong;
  else {
  ...
  if ((field = GetFieldStructure(ifd, ImageWidth)) == NULL) return (ERROR);

  /* get image width */
  if (field->Type == SHORT)
    *width = (int32_t)field->Value.UShort;
  else if (field->Type == LONG)
    *width = (int32_t)field->Value.ULong;
  else {
    WrongValueType("GetHeightAndWidth", field->Tag, field->Type);
    return (ERROR);
  }
...
static int32_t GetCompression(struct IFD *ifd, char *compress_type) {
  ...
  if (GetUShortValueFromField(ifd, Compression, &CompressionValue) == ERROR)
    return (ERROR);
...
static int32_t IsImageFullColor(struct IFD *ifd) {
  ...
  if ((field = GetFieldStructure(ifd, Compression)) == NULL) return (NO);
  if ((field->Value.UShort != NoCompression) &&
      (field->Value.UShort != PackBits))
    return (NO);

  /* PhotometricInterpretation must have 2 as a value */
  if ((field = GetFieldStructure(ifd, PhotometricInterpretation)) == NULL)
    return (NO);
  if (field->Value.UShort != 2) return (NO);

  /* SamplesPerPixel must not have a value less than 3 */
  if ((field = GetFieldStructure(ifd, SamplesPerPixel)) == NULL) return (NO);
  if (field->Value.UShort < 3) return (NO);

  /* ResolutionUnit must have 1 or 2 or 3 as a value */
  if ((field = GetFieldStructure(ifd, ResolutionUnit)) == NULL) return (NO);
  if ((field->Value.UShort != NoAbsoluteUnit) &&
      (field->Value.UShort != Inch) && (field->Value.UShort != Centimeter))
    return (NO);

  /* BitsPerSample must have 8, 8, 8 as values */
  if ((field = GetFieldStructure(ifd, BitsPerSample)) == NULL) return (NO);
  if (field->Count != 3)
    return (NO);
...
static int32_t IsImagePaletteColor(struct IFD *ifd) {
  ...
  /* ResolutionUnit must have 1 or 2 or 3 as a value */
  if ((field = GetFieldStructure(ifd, ResolutionUnit)) == NULL) return (NO);
  if ((field->Value.UShort != 1) && (field->Value.UShort != 2) &&
      (field->Value.UShort != 3))
    return (NO);

  /* BitsPerSample must have 4 or 8 as a value */
  if ((field = GetFieldStructure(ifd, BitsPerSample)) == NULL) return (NO);
  if ((field->Value.UShort != 4) && (field->Value.UShort != 8)) return (NO);
...
static int32_t IsImageGrayscale(struct IFD *ifd) {
  ...
  if ((field = GetFieldStructure(ifd, PhotometricInterpretation)) == NULL)
    return (NO);
  if ((field->Value.UShort != 0) && (field->Value.UShort != 1)) return (NO);
...
static int32_t WhatAboutCoreField(struct IFD *ifd, uint16_t Tag) {
  if (IsThereAFieldFor(ifd, Tag) == NO) {
    if (IsThereADefaultFor(Tag) == YES) {
      if (AddDefaultField(ifd, Tag) == ERROR) return (ERROR);
    } else {
...
static int32_t AddDefaultField(struct IFD *ifd, uint16_t Tag) {
  if (Tag == ResolutionUnit) return (MakeResolutionUnitField(ifd));
  if (Tag == RowsPerStrip) return (MakeDefaultRowsPerStripField(ifd));
...
static int32_t SeeIfThereAreOtherIFDs(FILE *fp, uint32_t OffsetOfIFD,
  ...
  PositionValue = OffsetOfIFD + 2 + ((uint32_t)(NumFieldsInInputFile)*12);
  ...
  if (IsTagRecognized(TempTag) == YES) {
    ...
    if (IsTypeRecognized(TempType) == YES) {
      ...
      if (IsTypeExpectedWithTag(TempTag, TempType) == YES) {
        ...
        if (AllocateAndCopyField(fp, ifd, TempTag, TempType) == ERROR)
          return (ERROR);
...
static int32_t AllocateAndCopyField(FILE *fp, struct IFD *ifd, uint16_t Tag,
  ...
  if (CopyField(fp, &(ifd->Fields[ifd->NumberOfFields - 1]), Tag, Type) ==
      ERROR) {
...
static int32_t GetArrayOfValues(FILE *fp, struct TIFF_field *field) {
  ...
  if (field->Type == BYTE) {
    for (i = 0; i < field->Count; i++)
      if (GetUnsignedChar(fp, &(field->Value.UCharArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == SHORT) {
    for (i = 0; i < field->Count; i++)
      if (GetUnsignedShort(fp, &(field->Value.UShortArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == LONG) {
    for (i = 0; i < field->Count; i++)
      if (GetUnsignedLong(fp, &(field->Value.ULongArray[i])) == ERROR)
        return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == RATIONAL) {
    for (i = 0; i < field->Count; i++)
      if (GetRational(fp, &(field->Value.RatArray[i])) == ERROR) return (ERROR);
    return (NO_ERROR);
  }
...
static int32_t GetSingleValue(FILE *fp, struct TIFF_field *field) {
  if (field->Type == BYTE) {
    if (GetUnsignedChar(fp, &(field->Value.UChar)) == ERROR) return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == SHORT) {
    if (GetUnsignedShort(fp, &(field->Value.UShort)) == ERROR) return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == LONG) {
    if (GetUnsignedLong(fp, &(field->Value.ULong)) == ERROR) return (ERROR);
    return (NO_ERROR);
  } else if (field->Type == RATIONAL) {
    if (GetRational(fp, &(field->Value.Rat)) == ERROR) return (ERROR);
    return (NO_ERROR);
  }
...
static int32_t AllocateArrayOfValues(struct TIFF_field *field) {
  if (field->Type == BYTE)
    if ((field->Value.UCharArray =
             (uint8_t *)calloc((size_t)field->Count, sizeof(uint8_t))) == NULL)
      return (ERROR);

  if (field->Type == SHORT)
    if ((field->Value.UShortArray = (uint16_t *)calloc(
             (size_t)field->Count, sizeof(uint16_t))) == NULL)
      return (ERROR);

  if (field->Type == LONG)
    if ((field->Value.ULongArray = (uint32_t *)calloc(
             (size_t)field->Count, sizeof(uint32_t))) == NULL)
      return (ERROR);

  if (field->Type == RATIONAL)
    if ((field->Value.RatArray = (struct Rational *)calloc(
             (size_t)field->Count, sizeof(struct Rational))) == NULL)
      return (ERROR);
...
static int32_t AllocateNewField(struct IFD *ifd) {
  if ((ifd->Fields = (struct TIFF_field *)realloc(
           (struct TIFF_field *)ifd->Fields,
           (size_t)(ifd->NumberOfFields * sizeof(struct TIFF_field)))) ==
      NULL) {
...
static int32_t GetRational(FILE *fp, struct Rational *Rat) {
  ...
  if (2 != fscanf(fp, "%4c%4c", (char *)(&numerator), (char *)(&denumerator))) {
...
static int32_t CheckTypeSizes(void) {
  if ((1 != sizeof(uint8_t)) || (2 != sizeof(uint16_t)) ||
      (4 != sizeof(uint32_t))) {
