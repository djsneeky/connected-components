  ...
  for (i = 0; i < input_img.height; i++)
    for (j = 0; j < input_img.width; j++) {
      ...
      if (pixel > 255) {
        ...
        if (pixel < 0)
          green_img.mono[i][j] = 0;
        else
          green_img.mono[i][j] = pixel;
      }
    }

  /* Illustration: constructing a sample color image -- interchanging the red
   * and green components from the input color image */
  for (i = 0; i < input_img.height; i++)
    for (j = 0; j < input_img.width; j++) {
